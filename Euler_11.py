...
grid_list = [row1, row2, row3, row4, row5, row6, row7, row8, row9, row10, row11, row12, row13, row14, row15, row16,
             row17, row18, row19, row20]


def right_max_production(max_production, row, i):
    test = 1
    if i > 16:
    ...
    for m in range(4):
        if i + m >= 20:
            break
        test *= grid_list[row][i + m]
    if test >= max_production:
        max_production = test
    return max_production


def down_max_production(max_production, row, i):
    test = 1
    if i > 16:
        return max_production
    for m in range(4):
        if i + m >= 20:
            break
        test *= grid_list[i + m][row]
    if test >= max_production:
        max_production = test
    return max_production


def diagonal_left_down_production(max_production, row, i):
    test = 1
    if i > 16 or row > 16:
        return max_production
    for m in range(4):
        if i + m >= 20:
            break
        test *= grid_list[row + m][i + m]
    if test >= max_production:
        max_production = test
    return max_production


def diagonal_right_down_production(max_production, row, i):
    test = 1
    if i < 3 or row > 16:
        return max_production
    for m in range(4):
...
for row in range(19):
    for i in range(19):
        new_max1 = right_max_production(a, row, i)
        new_max2 = down_max_production(a, row, i)
        new_max3 = diagonal_left_down_production(a, row, i)
        new_max4 = diagonal_right_down_production(a, row, i)
        new_max = max(new_max1, new_max2, new_max3, new_max4)
        if new_max >= a:
            a = new_max
